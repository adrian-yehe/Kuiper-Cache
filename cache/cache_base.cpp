...
namespace Kuiper {
    namespace Cache {
            ...
            //   addrRanges(p.addr_ranges.begin(), p.addr_ranges.end())
        {
            ...
            tags->tagsInit();
            // if (prefetcher)
            //     prefetcher->setParentInfo(getProbeManager(), getBlockSize());

            // cpuSidePort = p.cpu_side_port;
            // memSidePort = p.mem_side_port;
        }
            ...
            if (mshr) {
                ...
                if (pkt) {
                    ...
                    else
                    {
                        ...
                        spdlog::info("{:s} coalescing MSHR for {:s}", 
                                        __func__, pkt->print());
        ...
        BaseCache::recvTimingReq(PacketPtr pkt) {
            ...
            if (prefetcher) {
                ...
                //     prefetcher->nextPrefetchReadyTime(), lat);
                Tick next_pf_time = lat;
        ...
        BaseCache::recvTimingResp(PacketPtr pkt) {
            ...
            // gem5::panic_if(pkt->cmd != MemCmd::HardPFResp,
            //          "%s saw a non-zero packet delay\n", name());

            const bool is_error = pkt->isError();

            if (is_error) {
                spdlog::info("{:s}  Cache received {:s} with error", 
                        __func__, pkt->print());
            ...
            spdlog::info("{:s}: Handling response {:s}", 
                    __func__, pkt->print());
            ...
            if (is_fill && !is_error) {
                ...
                spdlog::info("Block for addr {:#x} being updated in Cache",
                        pkt->getAddr());
            ...
            const Tick forward_time = 0;
            // copy writebacks to write buffer
            // doWritebacks(writebacks, forward_time);

            // DPRINTF(CacheVerbose, "%s: Leaving with %s\n", __func__, pkt->print());
            spdlog::info("{:s}: Leaving with {:s}", __func__, pkt->print());
            delete pkt;
        }
        ...
        BaseCache::cmpAndSwap(CacheBlk *blk, PacketPtr pkt)
        {
            ...
            //     }
            //     else
            //         assert(0 && "Invalid size for conditional read/write\n");
            // }
                          ...
                          PacketList &writebacks)
        {
            ...
            assert((!(isReadOnly && pkt->isWrite())) && 
                        "Should never see a write in a read-only cache");
            ...
            if (pkt->isWriteback())
            {
                    ...
                    mshrQueue.findMatch(pkt->getAddr(), pkt->isSecure()))
                {
                    ...
                    spdlog::info("Clean writeback {:#x} to block with MSHR, "
                                   "dropping",
                            pkt->getAddr());
        ...
        BaseCache::handleFill(PacketPtr pkt, CacheBlk *blk, PacketList &writebacks,
            ...
            if (!pkt->hasSharers()) {
                ...
                if (pkt->cacheResponding())
                {
                    ...
                    assert(!isReadOnly && "Should never see dirty snoop response "
                                              "in read-only cache");

                    // gem5::gem5_assert(!isReadOnly, "Should never see dirty snoop response "
        ...
        BaseCache::allocateBlock(const PacketPtr pkt, PacketList &writebacks)
        {
            ...
            // insertion. Although in gem5::gem5 the data is stored uncompressed, even if a
        ...
        {
            assert((!isReadOnly || writebackClean) && "Writeback from read-only cache" );
            // gem5::gem5_assert(!isReadOnly || writebackClean,
        ...
        {
            // if (blk.isSet(CacheBlk::DirtyBit))
            //     gem5::warn_once("Invalidating dirty cache lines. "
        ...
        BaseCache::sendMSHRQueuePacket(MSHR *mshr) {
            ...
            if (writeAllocator && writeAllocator->coalesce() && tgt_pkt->isWrite()) {
                if (!mshr->isWholeLineWrite()) {
                    ...
                    if (writeAllocator->delay(mshr->blkAddr)) {
                        ...
                        //         tgt_pkt->print(), delay);
                        spdlog::info("Delaying pkt {:s} ticks to allow "
                                              "for write coalescing",
                                tgt_pkt->print());
            ...
            if (true == reqPacketQueue.sendTimingReq(pkt)) {
                ...
                if (pkt->isClean() && blk && blk->isSet(CacheBlk::DirtyBit)) {
                    ...
                    spdlog::info("{:s}: packet {:s} found block: {:s}",
                            __func__, pkt->print(), blk->print());
